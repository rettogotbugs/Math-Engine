...
export default function App() {
  ...
  const filteredTools = useMemo(() => {
    ...
    if (activeCategory === "favorites") {
      ...
      tools = mathTools.filter(
        (t) => t.category === activeCategory || t.classLevel === activeCategory,
      );
    }
    ...
    return tools;
  }, [activeCategory, searchQuery, sortBy, favorites, history]);
  ...
  const currentCategoryObj =
    ...
    CLASSES.find((c) => c.id === activeCategory);
                  ...
                  <div>
                    <h1 className="text-3xl font-bold tracking-tight text-white md:text-4xl flex items-center gap-3">
                      {currentCategoryObj?.icon && (
                        <currentCategoryObj.icon className="h-8 w-8 text-indigo-400" />
                      )}
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto shrink-0">
                    ...
                    <div className="relative w-full sm:w-auto">
                      <select
                        value={sortBy}
                        onChange={(e) =>
                          setSortBy(e.target.value as "a-z" | "popular")
                        }
