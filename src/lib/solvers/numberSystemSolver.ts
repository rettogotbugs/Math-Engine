export const numberSystemSolver = {
  isPrime: (n: number) => {
    ...
    const steps = [`Check if $${n}$ is prime.`, `Check divisibility up to $\\sqrt{${n}} \\approx ${limit.toFixed(2)}$`];
    
    for (let i = 3; i <= limit; i += 2) {
      if (n % i === 0) {
        steps.push(`$${n}$ is divisible by $${i}$ ($${i} \\times ${n / i} = ${n}$).`);
        return { result: "Not Prime", steps };
      }
    }
    steps.push(`No divisors found up to $\\sqrt{${n}}$.`);
  ...
  factorial: (n: number) => {
    ...
    let result = 1;
    const steps = [`Calculate $${n}!$`, `Formula: $n! = n \\times (n-1) \\times \\dots \\times 1$`];
